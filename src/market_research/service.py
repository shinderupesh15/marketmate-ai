...
from market_research.schemas import CreatorBrief, utcnow
...
class ResearchService:
    def __init__(self, data_dir=None, factory=None):
        ...
        self.db = self.data_dir / "creatorkit.sqlite"
    ...
    def session(self, run_id):
        ...
        if self.factory:
            ...
            agents = Agents(Models(settings, usage), YouSearch(settings.ydc_api_key.get_secret_value(), usage), usage)
        with SqliteSaver.from_conn_string(str(self.db)) as saver:
            yield build_graph(agents, saver), usage

    def new_run(self, brief: CreatorBrief):
        run_id = uuid4().hex
        usage = Usage(self.db, run_id)
