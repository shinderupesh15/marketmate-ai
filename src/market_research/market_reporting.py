...
from market_research.reporting import claim_text, text
...
def render_market_report(state):
    ...
    lines = [
        "# MarketMate AI",
        "",
        "**Approved**" if state.get("approved") else "**Draft — awaiting review**",
        "",
        "Business idea: " + text(brief["goal"]),
        "Audience: " + text(brief["audience"]),
        "Market: " + text(brief["country"]),
    ...
    for raw in state.get("profiles", {}).values():
        ...
        lines += ["", "**Dated news**"] + [
            f"- {n.published_date}: {claim_text(n.evidence)}" for n in p.news
        ]
        for gap in p.gaps:
    ...
    lines += [
        f"- Day {a['day']}: {text(a['task'])} — Deliverable: {text(a['deliverable'])}"
        for a in plan.get("actions", [])
    ]
    lines += ["", "## Assumptions to validate", ""] + [
        "- " + text(q) for q in plan.get("questions", [])
    ]
    lines += [
        "",
        "## Limitations",
        "",
        "Public evidence is incomplete. Brand claims are not independently proven. Suggestions are hypotheses, not guaranteed outcomes.",
    ]
