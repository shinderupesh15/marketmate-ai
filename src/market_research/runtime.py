...
class Usage:
    def __init__(self, db: Path, run_id: str, max_search: int = 30, max_model: int = 36):
        ...
        with self.connect() as c:
            c.execute("CREATE TABLE IF NOT EXISTS events (run_id TEXT, kind TEXT, detail TEXT, created TEXT)")
            c.execute("CREATE TABLE IF NOT EXISTS runs (id TEXT PRIMARY KEY, label TEXT, created TEXT)")
    @contextmanager
    def connect(self):
    ...
    def reserve(self, kind):
        ...
        with self.connect() as c:
            ...
            count = c.execute("SELECT COUNT(*) FROM events WHERE run_id=? AND kind=?",
                              (self.run_id, kind)).fetchone()[0]
            if count >= limit:
                raise BudgetExceeded(f"{kind.title()} request budget reached; completed evidence is preserved.")
            c.execute("INSERT INTO events VALUES (?,?,?,?)",
                      (self.run_id, kind, "Request reserved", datetime.now(timezone.utc).isoformat()))
    def counts(self):
        with self.connect() as c:
            return dict(c.execute("SELECT kind, COUNT(*) FROM events WHERE run_id=? GROUP BY kind",
                                  (self.run_id,)).fetchall())
    def events(self):
        with self.connect() as c:
            return c.execute("SELECT kind, detail, created FROM events WHERE run_id=? ORDER BY rowid",
                             (self.run_id,)).fetchall()
