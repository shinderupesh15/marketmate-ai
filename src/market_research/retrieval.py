...
def pricing_source(source):
    return any(
        term in source["url"].lower()
        for term in ("pricing", "price", "subscription", "payment", "billing", "/plans")
    )
...
def select_sources(ids, sources, company_url, limit=12):
    ...
    for sid in ordered:
        ...
        if previous is None or (src["retrieved_at"], len(src["text"])) >= (
            sources[previous]["retrieved_at"],
            len(sources[previous]["text"]),
        ):
            latest[src["url"]] = sid
    ...
    primary_prices = [
        sid
        for sid in ordered
        if official_source(sources[sid]["url"], company_url) and pricing_source(sources[sid])
    ]
    primary_prices.sort(
        key=lambda sid: (len(sources[sid]["text"]) > 400, sources[sid]["retrieved_at"]),
        reverse=True,
    )
    ...
    ranked = sorted(
        ordered,
        key=lambda sid: (
            official_source(sources[sid]["url"], company_url),
            sources[sid]["retrieved_at"],
        ),
        reverse=True,
    )
