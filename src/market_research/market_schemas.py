...
from market_research.schemas import Model, Claim, NewsItem, public_url

class BusinessBrief(Model):
    goal: str = Field(default="Launch an online healthy-snack brand for office workers", min_length=5, max_length=1000)
    audience: str = Field(default="Office workers looking for convenient snacks", min_length=3, max_length=500)
...
def empty_profile(name, url, brief, reason):
    return BusinessProfile(name=name, url=url, positioning=None, products=[], messaging=[],
                           pricing=[], news=[], gaps=[reason])
