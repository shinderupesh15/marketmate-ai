...
from market_research.runtime import BudgetExceeded, ServiceError, Usage
...
def test_retry_and_quota(tmp_path):
    ...
    search = YouSearch(
        "fake", usage, httpx.MockTransport(lambda r: httpx.Response(401)), sleep=lambda _: None
    )
        ...
        if request.url.path.endswith("contents"):
            return httpx.Response(
                ...
                json=[
                    {
                        "url": "https://example.com/pricing",
                        "markdown": "Pro is INR 500 monthly.",
                        "title": "Plans",
                    }
                ],
            )
