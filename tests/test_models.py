    ...
    class LLM:
        ...
        def invoke(self, messages):
            if len(limits) == 1 and sdk_exception:
                ...
                raise error
            raw = SimpleNamespace(usage_metadata={}, response_metadata={"finish_reason":"length" if len(limits)==1 else "stop"})
...
def test_invalid_output_remains_bounded_and_sanitized(tmp_path):
    class LLM:
        ...
        def invoke(self, messages): raise ValueError("private payload")
    models = Models.__new__(Models)
    models.llm = LLM()
    models.usage = Usage(tmp_path/"usage.sqlite", "test")
